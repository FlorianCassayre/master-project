...
This general problem happens to relate to the area of compiler optimizations, in particular kernel proofs could be thought as a sequence of static single assignments, where each value would be a call to some function taking previously computed values as arguments (the premises).
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption[YYY]{Example of applying the flattening procedure on a proof. The first graph represents the original proof while the second corresponds to the final state after flattening. The nodes in gray are the imports that have been inlined.}
...
Some of the optimizations that are currently in place are listed in figure \ref{fig:synthesis-simplifications}.
...
\begin{figure}[H]
  ...
  \hypo{\Gamma \vdash \Delta}
  \infer1[Rewrite]{\Gamma'' \vdash \Delta''}
  \end{prooftree} \label{eq:simplify-rewrite-rewrite} \\[1em]
  ...
  \end{prooftree} \label{eq:simplify-hypothesis-weakening} \\[1em]
  % Cut left
  \begin{prooftree}
  ...
  \end{prooftree} \label{eq:simplify-cut-left}
  \end{gather}
  \caption[Sample term rewriting rules]{Sample of term rewriting rules used to simply a proof. \eqref{eq:simplify-rewrite-rewrite}} corresponds to double rewriting elimination; this rule exists in different flavours, to cover weakening as well. \eqref{eq:simplify-hypothesis-weakening} accounts for hypotheses followed by weakening (or rewrites). \eqref{eq:simplify-cut-left} eliminates needless cut rule applications.

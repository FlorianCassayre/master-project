...
\end{prooftree}
  \caption[Proof in LISA (2)]{Translation of the proof in \autoref{lst:proof-justifications} into LISA. The left branch was derived in forward mode (forward rule application followed by an implicit instantiation), while the remaining steps were derived in backward mode through the tactical \code{apply} steps.}
